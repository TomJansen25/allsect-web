...
3.  **`/src`**: This directory contains all the code for the front-end of the Allsect website, with the following sub-directories:
    1.  **`/backup`**: react components or gatsby pages that are currently not used but are kept for potentially later use
...
4.  **`.env.example`**: Exemplary .env file containing all the required environment variables to run everything on the website smoothly. Replace all `insertValue` values with actual variables and rename file to `.env`
   
5.  **`.firebaserc`**: This file defines the Firebase CLI settings

6.  **`.gitignore`**: This file tells git which files it should not track / not maintain a version history for.

7.  **`.prettierrc`**: This is a configuration file for [Prettier](https://prettier.io/). Prettier is a tool to help keep the formatting of your code consistent.

8.  **`firebase.json`**: This file is the configuration file for [Firebase](https://firebase.google.com/) that defines the settings for Firebase Hosting.
